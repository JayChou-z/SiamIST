...
class seq_22Dataset(Dataset):
    def __init__(self, seq_dataset, z_transforms, x_transforms, name = 'sequence-22'):
    ...
    def open(self):
        ...
        template_img = Image.open(self.ret['template_img_path'])   #pil  读取到的是图片对象
        ...
        exemplar_img, scale_z, s_z, w_x, h_x = self.get_exemplar_image( template_img,
                                                                        ...
                                                                        config.context_amount, img_mean ) #模板图片 中心  127/255 0,。5 图片均值
    ...
    def crop_and_pad(self, img, cx, cy, gt_w, gt_h, a_x, b_y, model_sz, original_sz, img_mean=None):
        ...
        if not np.array_equal(model_sz, original_sz):
            ...
            # s = SideWindowFilter(radius=4, iteration=10)
...
if __name__ == "__main__":  #直接运行时
    ...
    root_dir = '../data/22data'
    seq_dataset = seqs_22_train(root_dir, subset='train')
    train_data  = seq_22Dataset(seq_dataset)
